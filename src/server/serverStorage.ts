import { getDb, ensureSchemaAndSeed, isNeonQuotaError, markNeonQuotaExceeded, detectProvider, getConnectionString } from '../db/index';
...
export interface ServerState {
  ...
  lastUpdated: string;
  source: 'neon_postgresql' | 'supabase_postgresql' | 'postgresql' | 'google_firestore' | 'memory_preview';
}

class ServerStorageService {
  // Ephemeral in-memory state used when live databases are not yet configured or in fallback
  private memoryState: ServerState;
  private cachedBootstrap: { data: ServerState; timestamp: number } | null = null;
  ...
  private handleNeonError(context: string, err: any) {
  ...
  public async getBootstrapData(forceRefresh = false): Promise<ServerState> {
    if (!forceRefresh && this.cachedBootstrap && Date.now() - this.cachedBootstrap.timestamp < 15000) {
    ...
    if (db) {
      try {
        ...
        // Query tables sequentially / in small groups to prevent connection pool exhaustion on Supabase/Neon poolers
        const depList = await db.select().from(dbDepartments);
        const lvlList = await db.select().from(dbLevels);
        const crsList = await db.select().from(dbCourses);
        const trnList = await db.select().from(dbTrainers);
        const untList = await db.select().from(dbUnits);
        const catList = await db.select().from(dbUnitCategories);
        const serList = await db.select().from(dbAssessmentSeries);
        const usrList = await db.select().from(dbUsers);
        const stuList = await db.select().from(dbStudents);
        const regList = await db.select().from(dbRegistrations).orderBy(desc(dbRegistrations.submittedAt));
        const notList = await db.select().from(dbNotifications).orderBy(desc(dbNotifications.createdAt));
        const cfgList = await db.select().from(dbInstitutionConfig).limit(1);
        const logList = await db.select().from(dbAuditLogsGlobal).orderBy(desc(dbAuditLogsGlobal.timestamp)).limit(200);

        const resultState: ServerState = {
          ...
          source: (() => {
            ...
            if (c) {
              ...
              if (p.includes('supabase')) return 'supabase_postgresql' as const;
              if (p.includes('neon')) return 'neon_postgresql' as const;
              return 'postgresql' as const;
            }
            return 'neon_postgresql' as const;
          })(),
        };
    ...
    // Try Google Cloud Firestore fallback / persistent storage
    try {
      ...
      if (fsData && (fsData.departments.length > 0 || fsData.users.length > 0 || fsData.registrations.length > 0)) {
        ...
        // Seed baseline data to Firestore in background so it is ready
  ...
  public async syncFromFirestoreToPostgres(): Promise<{
    success: boolean;
    usersCount: number;
